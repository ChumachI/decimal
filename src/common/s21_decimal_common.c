...
int _divide_uint(my_decimal *value, uint32_t num, int round_type) {
  ...
  if (num == 0) {
    ...
    my_decimal result = {0};  // Сюда записываем результат деления
    for (int i = 2; i >= 0; i--) {
...
void _set_scale(my_decimal *value, int exp) {
  ...
  if (dst == NULL) {
    ...
    if (_is_zero(src) == 1 && (src.bits[3] & 0x7FFFFFFF) == 0) {
      ...
      *dst = sign_bit ? 0.0f * -1 : 0.0f;
      ...
      temp = temp * powl(10, -my_get_scale(src));
      if (sign_bit) temp = temp * (-1);
      // Проверка на наличие больших, малых или бесконечных значений
      if ((temp != 0.0 && fabsl(temp) < 1.0e-28) || fabsl(temp) > my_MAXDEC) {
        *dst = 0.0L;
        return_status = 1;
      } else {
